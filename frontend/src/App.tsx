...
function LeaderboardPage({onOpenProfile}:{onOpenProfile:(name:string)=>void}) {
  const max=leaderboardReaders[0].books;
  const podium=[leaderboardReaders[1],leaderboardReaders[0],leaderboardReaders[2]];
  return <section className="feature-page leaderboard-page">
    <div className="leaderboard-hero"><div><p className="eyebrow accent">Friends · August</p><h2>Leaderboard</h2><p>A friendly books-finished ranking. Pages and reading time add context—there are no streaks here.</p></div><div className="period-switch"><button className="active">This month</button><button>This year</button><button>All time</button></div></div>
    <div className="feature-metrics">{pageContent.Leaderboard.cards.map((card,index)=><article key={card.label}><span className={`feature-number tone-${index+1}`}>{index+1}</span><div><p>{card.label}</p><strong>{card.value}</strong><small>{card.note}</small></div></article>)}</div>
    <section className="podium-panel"><div className="podium-title"><div><p className="eyebrow">Monthly standings</p><h3>Books finished</h3></div><span><Medal size={17}/> Read at your own pace</span></div><div className="podium">{podium.map((reader,index)=>{const rank=index===0?2:index===1?1:3;return <button className={`podium-person place-${rank}`} key={reader.name} onClick={()=>onOpenProfile(reader.name === "Alex" ? "Alex Morgan" : reader.name)}><div className="podium-avatar"><img src={reader.avatar} alt={`${reader.name} profile`}/><span>{rank}</span></div><strong>{reader.name}</strong><small>{reader.books} books finished</small><div><b>{reader.pages.toLocaleString()} pages</b><b>{reader.time}</b></div></button>})}</div></section>
    <section className="leaderboard-table"><header><div><p className="eyebrow">Your circle</p><h3>Full standings</h3></div><span>Ranked by books finished</span></header>{leaderboardReaders.map((reader,index)=><button key={reader.name} onClick={()=>onOpenProfile(reader.name === "Alex" ? "Alex Morgan" : reader.name)}><b className={`leader-rank rank-${index+1}`}>{index+1}</b><img src={reader.avatar} alt=""/><span className="leader-name"><strong>{reader.name}</strong><small>Recently finished {reader.last}</small></span><span className="leader-bar"><i style={{width:`${reader.books/max*100}%`}}/></span><strong className="leader-pages">{reader.books} books</strong><span className="leader-context">{reader.pages.toLocaleString()} pages · {reader.time}</span><ChevronRight size={17}/></button>)}</section>
  </section>;
}
